...
=partial-recall-non-meaningful-explainer= is set which is ready by
=partial-recall-explain-omission= if you want to know why a buffer was
omitted (also present in the help echo of the mode-line lighter). So
if you add your own trait here, consider doing the following:
