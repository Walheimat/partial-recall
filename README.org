...
meaningful buffer. By default only =buffer-file-name= is in the list,
meaning any buffer that has a file name is meaningful. If you set it
to ='(always)= all buffers would be considered meaningful.
